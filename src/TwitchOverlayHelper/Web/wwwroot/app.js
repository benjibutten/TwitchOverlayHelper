...
/* The inside of a message – words, emotes, links, markers – is drawn by chat-render.js, which the
   stream overlay loads as well: a link chip or a gigantified emote that looked one way here and
   another way on the broadcast would be the same message telling two stories. What stays in this
   file is everything around it, which is the half that is the dock's alone. */
...
function build(message) {
  ...
  if (state.settings.showBadges) appendBadges(head, message.badges);
  ...
  node.appendChild(renderBody(message, state.settings));
...
function buildEvent(chatEvent) {
  ...
  if (state.settings.showTimestamps) {
    const time = document.createElement("span");
    time.className = "msg-time";
    time.textContent = clockOf(chatEvent.at);
    head.appendChild(time);
  }
  ...
  if (chatEvent.message) node.appendChild(renderBody({ text: chatEvent.message, emotes: chatEvent.emotes, isAction: false }, state.settings));
  return node;
}
